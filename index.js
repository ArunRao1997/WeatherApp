...
async function fetchData(target) {
    try {
        let url = `https://api.weatherapi.com/v1/current.json?key=7a299f9a430b4030894161446231305&q=${target}&aqi=no`
...
function updateDOM(temp, locationName, time, emoji, condition) {
    ...
    // Split the exactdate string into its components
    const [year, month, day] = exactdate.split('-');

    // Created a new date string in the expected format
    const formattedDate = `${month}/${day}/${year}`;

    // formattedDate string to create the Date object
    const exactDay = getDayFullName(new Date(formattedDate).getDay());
